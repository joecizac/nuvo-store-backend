...
*   **Hibernate 6/7 HQL Limitations:** We discovered that Hibernate's HQL parser has difficulty handling PostGIS boolean return types. We implemented **Native SQL Queries** in the `StoreRepository` to ensure reliability and performance.
...
2.  **Image Upload Service:** Create a unified `FileStorageService` to handle binary uploads for store logos and product images.
